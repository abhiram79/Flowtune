package com.abhiram.flowtube

import com.abhiram.flowtube.models.AccountInfo
...
import com.abhiram.flowtube.models.SearchSuggestions
import com.abhiram.flowtube.models.SongItem
import com.abhiram.flowtube.models.WatchEndpoint
import com.abhiram.flowtube.models.WatchEndpoint.WatchEndpointMusicSupportedConfigs.WatchEndpointMusicConfig.Companion.MUSIC_VIDEO_TYPE_ATV
import com.abhiram.flowtube.models.YouTubeClient.Companion.ANDROID_MUSIC
import com.abhiram.flowtube.models.YouTubeClient.Companion.IOS
import com.abhiram.flowtube.models.YouTubeClient.Companion.WEB
import com.abhiram.flowtube.models.YouTubeClient.Companion.WEB_REMIX
import com.abhiram.flowtube.models.YouTubeLocale
import com.abhiram.flowtube.models.getContinuation
import com.abhiram.flowtube.models.oddElements
import com.abhiram.flowtube.models.response.AccountMenuResponse
import com.abhiram.flowtube.models.response.BrowseResponse
import com.abhiram.flowtube.models.response.GetQueueResponse
import com.abhiram.flowtube.models.response.GetSearchSuggestionsResponse
import com.abhiram.flowtube.models.response.GetTranscriptResponse
import com.abhiram.flowtube.models.response.NextResponse
import com.abhiram.flowtube.models.response.PipedResponse
import com.abhiram.flowtube.models.response.PlayerResponse
import com.abhiram.flowtube.models.response.SearchResponse
import com.abhiram.flowtube.pages.AlbumPage
import com.abhiram.flowtube.pages.AlbumUtils
import com.abhiram.flowtube.pages.ArtistItemsContinuationPage
import com.abhiram.flowtube.pages.ArtistItemsPage
import com.abhiram.flowtube.pages.ArtistPage
import com.abhiram.flowtube.pages.BrowseResult
import com.abhiram.flowtube.pages.ExplorePage
import com.abhiram.flowtube.pages.HomeAlbumRecommendation
import com.abhiram.flowtube.pages.HomePlayList
import com.abhiram.flowtube.pages.MoodAndGenres
import com.abhiram.flowtube.pages.NewReleaseAlbumPage
import com.abhiram.flowtube.pages.NextPage
import com.abhiram.flowtube.pages.NextResult
import com.abhiram.flowtube.pages.PlaylistContinuationPage
import com.abhiram.flowtube.pages.PlaylistPage
import com.abhiram.flowtube.pages.RecommendationAlbumBundle
import com.abhiram.flowtube.pages.RelatedPage
import com.abhiram.flowtube.pages.SearchPage
import com.abhiram.flowtube.pages.SearchResult
import com.abhiram.flowtube.pages.SearchSuggestionPage
import com.abhiram.flowtube.pages.SearchSummary
...
/**
 * Parse useful data with [flowtube] sending requests.
 * Modified from [ViMusic](https://github.com/vfsfitvnm/ViMusic)
 */

object YouTube {
    private val flowtube = flowtube()

    var locale: YouTubeLocale
        get() = flowtube.locale
        set(value) {
            flowtube.locale = value
        }
    var visitorData: String
        get() = flowtube.visitorData
        set(value) {
            flowtube.visitorData = value
        }
    var cookie: String?
        get() = flowtube.cookie
        set(value) {
            flowtube.cookie = value
        }
    var proxy: Proxy?
        get() = flowtube.proxy
        set(value) {
            flowtube.proxy = value
        }

    suspend fun searchSuggestions(query: String): Result<SearchSuggestions> =
        runCatching {
            val response = flowtube.getSearchSuggestions(WEB_REMIX, query).body<GetSearchSuggestionsResponse>()
            SearchSuggestions(
                queries =
                    response.contents
                        ?.getOrNull(0)
    ...
    suspend fun searchSummary(query: String): Result<SearchSummaryPage> =
        runCatching {
            val response = flowtube.search(WEB_REMIX, query).body<SearchResponse>()
            SearchSummaryPage(
                summaries =
                    response.contents
    ...
    suspend fun search(
        ...
        runCatching {
            val response = flowtube.search(WEB_REMIX, query, filter.value).body<SearchResponse>()
            SearchResult(
                items =
                    response.contents
                        ?.tabbedSearchResultsRenderer
    ...
    suspend fun searchContinuation(continuation: String): Result<SearchResult> =
        runCatching {
            val response = flowtube.search(WEB_REMIX, continuation = continuation).body<SearchResponse>()
            SearchResult(
                items =
    ...
    suspend fun album(
        ...
        runCatching {
            val response = flowtube.browse(WEB_REMIX, browseId).body<BrowseResponse>()
            val playlistId =
    ...
    suspend fun albumSongs(playlistId: String): Result<List<SongItem>> =
        runCatching {
            val response = flowtube.browse(WEB_REMIX, "VL$playlistId").body<BrowseResponse>()
            response.contents
                ?.twoColumnBrowseResultsRenderer
                ?.secondaryContents
                ?.sectionListRenderer
    ...
    suspend fun artistItems(endpoint: BrowseEndpoint): Result<ArtistItemsPage> =
        runCatching {
            val response = flowtube.browse(WEB_REMIX, endpoint.browseId, endpoint.params).body<BrowseResponse>()
            val gridRenderer =
                response.contents
                    ?.singleColumnBrowseResultsRenderer
                    ?.tabs
    ...
    suspend fun artistItemsContinuation(continuation: String): Result<ArtistItemsContinuationPage> =
        runCatching {
            val response = flowtube.browse(WEB_REMIX, continuation = continuation).body<BrowseResponse>()
    ...
    suspend fun explore(): Result<ExplorePage> =
        runCatching {
            val response = flowtube.browse(WEB_REMIX, browseId = "FEmusic_explore").body<BrowseResponse>()
            ExplorePage(
                newReleaseAlbums =
    ...
    suspend fun newReleaseAlbums(): Result<List<AlbumItem>> =
        runCatching {
            val response = flowtube.browse(WEB_REMIX, browseId = "FEmusic_new_releases_albums").body<BrowseResponse>()
            response.contents
                ?.singleColumnBrowseResultsRenderer
                ?.tabs
                ?.firstOrNull()
    ...
    suspend fun moodAndGenres(): Result<List<MoodAndGenres>> =
        runCatching {
            val response = flowtube.browse(WEB_REMIX, browseId = "FEmusic_moods_and_genres").body<BrowseResponse>()
            response.contents
                ?.singleColumnBrowseResultsRenderer
                ?.tabs
                ?.firstOrNull()
    ...
    suspend fun recommendAlbum(
        ...
        runCatching {
            val response = flowtube.browse(WEB_REMIX, browseId = browseId).body<BrowseResponse>()
    ...
    suspend fun home(): Result<List<HomePlayList>> =
        runCatching {
            val response = flowtube.browse(WEB_REMIX, browseId = "FEmusic_home").body<BrowseResponse>()
            val continuation =
                response.contents
                    ?.singleColumnBrowseResultsRenderer
                    ?.tabs
    ...
    suspend fun browse(
        ...
        runCatching {
            val response = flowtube.browse(WEB_REMIX, browseId = browseId, params = params).body<BrowseResponse>()
            BrowseResult(
                title =
                    response.header
                        ?.musicHeaderRenderer
    ...
    suspend fun player(
        ...
        runCatching {
            ...
            if (this.cookie != null && false) { // if logged in: try ANDROID_MUSIC client first because IOS client does not play age restricted songs
                playerResponse = flowtube.player(ANDROID_MUSIC, videoId, playlistId).body<PlayerResponse>()
...
//            try {
//                val safePlayerResponse = flowtube.player(WEB_REMIX, videoId, playlistId).body<PlayerResponse>()
...
//            }
            ...
            playerResponse = flowtube.player(IOS, videoId, playlistId).body<PlayerResponse>()
            if (playerResponse.playabilityStatus.status == "OK") {
                return@runCatching playerResponse
            }
            val audioStreams = flowtube.pipedStreams(videoId).body<PipedResponse>().audioStreams
            playerResponse.copy(
                streamingData =
    ...
    suspend fun related(endpoint: BrowseEndpoint) =
        runCatching {
            val response = flowtube.browse(WEB_REMIX, endpoint.browseId).body<BrowseResponse>()
    ...
    suspend fun transcript(videoId: String): Result<String> =
        runCatching {
            val response = flowtube.getTranscript(WEB, videoId).body<GetTranscriptResponse>()
            response.actions
                ?.firstOrNull()
    ...
    suspend fun visitorData(): Result<String> =
        runCatching {
            Json
                .parseToJsonElement(flowtube.getSwJsData().bodyAsText().substring(5))
                .jsonArray[0]
                .jsonArray[2]
                .jsonArray
